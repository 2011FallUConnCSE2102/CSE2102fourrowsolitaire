...
public class SolitaireBoard extends JFrame
{
    ...
    private void dealOutCustomBoard(LinkedList<Integer> numbers, int numViewableCards)
    {
    ...
    public void setBackgroundNumber(int backNum)
    {
        ...
        if(backgroundNumber > ChangeAppearance.NUM_BACKGROUNDS || backgroundNumber <= 0)
        {
                ...
                for(int j = 0; j < 8; j++)
                {
                            ...
                            && !(destination instanceof SingleCell))
                    {
                        for(int k = temp.length() - 1; k >= 0; k--)
                        {
                            ...
                            && destination.isEmpty() && (source.getBottom().getNumber() != 13 || source instanceof SingleCell))
                    {
